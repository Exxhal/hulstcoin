...
#if defined(NDEBUG)
# error "Hulstcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3c6d0de7fe7a29f1b17297869c525e08c0ec60aff2de3073330096d749a0913c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hulstcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hulstcoin Signed Message:\n";
...
{
    // Hulstcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Hulstcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Hulstcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Hulstcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hulstcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hulstcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe605ba4788a5cad448b02cde7e094da526003a4f950c6d59fd8f2e1fab1c01e5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Try Firefox with the bookmarks, history and passwords from another browser";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d680cfcd494786f2c6c94c2ad6b63ab1faf568931fead9efe707cafc1780b4db526e70548705afa698958402f8447f90c630535bc2b3c75f8b318e1d92bf2cab") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1580115835;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085604751;

        if (fTestNet)
        {
            block.nTime    = 1580115814;
            block.nNonce   = 388644231;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc75fc3a36af4e1d1c16011ab096bf996a5eacd62b24a17ced271fe8569a52a1b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xbb, 0xda }; // Hulstcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HulstcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HulstcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HulstcoinMiner(CWallet *pwallet)
{
    printf("HulstcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hulstcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HulstcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HulstcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HulstcoinMiner, pwallet));
}
