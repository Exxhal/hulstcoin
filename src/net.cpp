...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Hulstcoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
 0x8dd85097,0xaa00a8c0,0x0100007f
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Hulstcoin is probably already running."), addrBind.ToString().c_str());
        else
